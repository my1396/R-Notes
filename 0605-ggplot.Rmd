...
```{r echo=FALSE}
fq_file <- "data/bootstrap_lagged_path_quantiles.csv"
LAGS <- 0:2
...
p_base <- pf +
  ...
  labs(
    title = "Global GDP impact with bootstrap uncertainty",
    subtitle = paste0(
      "SSP585", ", interactive specification, 1,000 draws. ",
...
f_name <- file.path("./images/", "p_base.png")
ggsave(f_name, p_base,
  width = 8.8, height = 5.4, dpi = 200
)
```

```{r fig.width=8.8, fig.height=5.4, out.width="100%"}
# Use default theme
p_base
```
...
p_mytheme <- p_base +
  my_theme
f_name <- file.path("./images/", "p_mytheme.png")
ggsave(f_name, p_mytheme,
  width = 8.8, height = 5.4, dpi = 200
)
```
