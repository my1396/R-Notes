...
- R Markdown style: Use `tag=value` in the chunk header ```` ```{r} ````.
  ...
  ~~~~markdown
  ```{r #fig-my-label, out.width="100%", fig.cap = caption}`r ''`
...
````markdown
```{r setup, child = "../_chunk-opt.qmd", include=FALSE}`r ''`
```
````
...
- GIF needs to be converted to static images for pdf output.
  ...
  `` `r knitr::inline_expr("if (knitr::is_html_output()) '<img src=\"images/icon-google-colab.svg\" alt=\"\" style=\"display: inline; height: 1.5em; vertical-align: bottom;\" />' else '\\\\raisebox{-0.2em}{\\\\includegraphics[height=1.5em]{images/icon-google-colab.png}}'")` ``
...
````markdown
```{r #fig-scatter, echo=FALSE, fig.cap="Scatter plots example", out.width="80%", fig.align="center", fig.pos="H"} `r ''`
...
````r
```{r alterative-models, echo=TRUE, eval=FALSE}`r ''`
library(stargazer)
...
```{r alterative-models-out, ref.label='alterative-models', echo=FALSE, results='asis'} `r ''`
```
...
```{r wide-table, echo=FALSE, results='asis'} `r ''`
library(knitr)
