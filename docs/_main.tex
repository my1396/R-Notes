...
We can load additional LaTeX packages using the \texttt{extra\_dependencies} option within the {\texttt{pdf\_document}} YAML settings.
...
The advantage of using the \texttt{extra\_dependencies} argument over the \texttt{includes} argument introduced in Section \href{https://bookdown.org/yihui/rmarkdown-cookbook/latex-preamble.html\#latex-preamble}{6.1} is that you do not need to include an external file, so your Rmd document can be self-contained.
...
Put citations in parentheses, use \texttt{{[}@citationkey{]}}.

To cite multiple entries, separate the keys by semicolons, e.g., \texttt{{[}@key-1;\ @key-2;\ @key-3{]}}. To suppress the mention of the author, add a minus sign before \texttt{@}, e.g., \texttt{{[}-@citationkey{]}}.
...
\textbf{Add an item to a bibliography without using it}
...
\item
  ...
  \item
    See \href{https://www.kenjisato.jp/en/post/2017/02/cross-referenceable-equation-with-preview-in-rmarkdown/}{this post} for a more efficient workaround.
  \end{itemize}
\item
...
\section{\texorpdfstring{Numbered \texttt{proof} environment}{Numbered proof environment}}\label{numbered-proof-environment}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{::: \{.theorem \#label name="Theorem name"\}}
\NormalTok{Here is my theorem.}
\NormalTok{:::}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \textbf{Theorem 12.1 (Pythagorean theorem)} For a right triangle, if \(c\) denotes the length of the hypotenuse and \(a\) and \(b\) denote the lengths of the other two sides, we have
...
\end{Shaded}
  ...
  Note that you have to use \texttt{exercise} and the correspoding label prefix \texttt{exr}.
\end{enumerate}
