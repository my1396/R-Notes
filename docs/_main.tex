...
\addcontentsline{toc}{subsubsection}{HTML Output}
...
\addcontentsline{toc}{subsubsection}{PDF Output}
...
\addcontentsline{toc}{subsubsection}{\texttt{header-includes}}
...
\InformationTok{  comment = "\#\textgreater{}", }
...
\NormalTok{knitr}\SpecialCharTok{::}\NormalTok{knit\_hooks}\SpecialCharTok{$}\FunctionTok{set}\NormalTok{(}\AttributeTok{small\_mar =} \ControlFlowTok{function}\NormalTok{(before, ...) \{}
  \ControlFlowTok{if}\NormalTok{ (before)}
    \FunctionTok{par}\NormalTok{(}\AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{, }\DecValTok{4}\NormalTok{, .}\DecValTok{1}\NormalTok{, .}\DecValTok{1}\NormalTok{))  }\CommentTok{\# smaller top/right margin}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# for meaningful output, the code below should be}
\CommentTok{\# executed *inside* a code chunk of a knitr document}
...
\CommentTok{\# or knitr::knit\_hooks$get(c(\textquotesingle{}source\textquotesingle{}, \textquotesingle{}output\textquotesingle{}))}
\end{Highlighting}
\end{Shaded}
...
\AnnotationTok{date:}\CommentTok{ \textquotesingle{}2025{-}05{-}17\textquotesingle{}}
...
Refer to the theorem using \texttt{\textbackslash{}@ref(prefix:label)}.
See the column \texttt{Label\ Prefix} in Table for the value of \texttt{prefix} for each environment.
E.g., see theorem \ref{thm:label} (\texttt{\textbackslash{}@ref(thm:label)}).
...
Alternatively, you can use the syntax based on Pandoc's \href{https://pandoc.org/MANUAL.html\#divs-and-spans}{fenced \texttt{Div} blocks}. It can already be used in any R Markdown document to write \href{https://bookdown.org/yihui/rmarkdown-cookbook/custom-blocks.html}{custom blocks.}
...
Apply Theorem @ref\{thm:pyth\}, \ldots{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\textasciigrave{}\textasciigrave{}}\InformationTok{\textasciigrave{}\{r out.width="50\%", fig.asp = 0.62, fig.cap="\textasciigrave{}}\NormalTok{out.width=\textbackslash{}"50\%\textbackslash{}"\textasciigrave{}, fig.asp set to 0.62."\}}
\FunctionTok{\# plot text is scaled too}
\NormalTok{plot(aapl$AAPL.Close)}
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}\{r fig.width=6, fig.asp=0.6\}}
\InformationTok{\# Text font does NOT scale, but figure title got cropped}
\InformationTok{plot(aapl$AAPL.Close)}
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{0206-Rmd-Figure_files/figure-latex/unnamed-chunk-3-1.pdf}}
\caption{\label{fig:unnamed-chunk-3}Set \texttt{fig.width}. Note that text font does NOT scale with figure, BUT the figure title got cropped.}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\textasciigrave{}\textasciigrave{}}\InformationTok{\textasciigrave{}\{r out.width="100\%", fig.asp = 0.6, fig.cap="\textasciigrave{}}\NormalTok{out.width=\textbackslash{}"100\%\textbackslash{}"\textasciigrave{}."\}}
\NormalTok{plot(aapl$AAPL.Close)}
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}}
\end{Highlighting}
\end{Shaded}
...
\textbackslash begin\{figure\}
\includegraphics[width=1\linewidth]{0206-Rmd-Figure_files/figure-latex/unnamed-chunk-4-1} \textbackslash caption\{\texttt{out.width="100\%"}, fig.asp set to 0.6. Note that the plot text got zoomed too, can be too large.\}\label{fig:unnamed-chunk-4}
\textbackslash end\{figure\}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f\_name }\OtherTok{\textless{}{-}} \StringTok{"images/aapl.png"}
\FunctionTok{png}\NormalTok{(f\_name, }\AttributeTok{width=}\DecValTok{2594}\NormalTok{, }\AttributeTok{height=}\DecValTok{1600}\NormalTok{, }\AttributeTok{res=}\DecValTok{300}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(aapl}\SpecialCharTok{$}\NormalTok{AAPL.Close)}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{0206-Rmd-Figure_files/figure-latex/secondplot-1.pdf}}
...
Note that you must provide \texttt{caption} option in \texttt{knitr::kable()}. Otherwise the table won't be numbered.
...
\AnnotationTok{date:}\CommentTok{ "2025{-}05{-}17"}
\AnnotationTok{output:}
...
\addcontentsline{toc}{subsubsection}{Text table}
...
\addcontentsline{toc}{subsubsection}{HTML table}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\textless{}!{-}{-} Chunk option settings {-}{-}\textgreater{}} 

\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}\{r chunk{-}opt, include=FALSE\}}
\InformationTok{\# set default chunk options}
\InformationTok{opts\_chunk$set(echo = TRUE, message=FALSE, fig.align="center", fig.pos = "H")}
\InformationTok{opts \textless{}{-} options(knitr.kable.NA = "")}
\InformationTok{\textasciigrave{}\textasciigrave{}\textasciigrave{}}

\CommentTok{\textless{}!{-}{-} Other shared Rmd snippets {-}{-}\textgreater{}} 
\NormalTok{...}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  See instructions \href{https://my1396.github.io/Econ-Study/2023/10/12/GitHub101.html\#push-an-existing-repository}{HERE}.
\item
...
\addcontentsline{toc}{subsection}{\texttt{.Rmd} files}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df }\OtherTok{\textless{}{-}}\NormalTok{ (}\FunctionTok{nrow}\NormalTok{(capm\_ml}\SpecialCharTok{$}\NormalTok{model)}\SpecialCharTok{{-}}\DecValTok{2}\NormalTok{)  }\CommentTok{\# degree of freedom}
\NormalTok{sigma2 }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(capm\_ml}\SpecialCharTok{$}\NormalTok{residuals}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\NormalTok{df  }\CommentTok{\# residual variance}
\CommentTok{\# 1st column of lm$model is the depend. var.}
\NormalTok{X }\OtherTok{\textless{}{-}}\NormalTok{ capm\_ml}\SpecialCharTok{$}\NormalTok{model[, }\DecValTok{2}\NormalTok{, drop}\OtherTok{=}\ConstantTok{FALSE}\NormalTok{] }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.matrix}\NormalTok{()}
\NormalTok{X }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(}\DecValTok{1}\NormalTok{, X)  }\CommentTok{\# add intercept}
\NormalTok{V }\OtherTok{\textless{}{-}} \FunctionTok{solve}\NormalTok{(}\FunctionTok{t}\NormalTok{(X) }\SpecialCharTok{\%*\%}\NormalTok{ X) }\SpecialCharTok{*}\NormalTok{ sigma2}
\NormalTok{V}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(capm\_ml)}\SpecialCharTok{$}\NormalTok{cov.unscaled}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{all.equal}\NormalTok{(}\FunctionTok{solve}\NormalTok{(}\FunctionTok{t}\NormalTok{(X) }\SpecialCharTok{\%*\%}\NormalTok{ X), }
          \FunctionTok{summary}\NormalTok{(capm\_ml)}\SpecialCharTok{$}\NormalTok{cov.unscaled, }
