...
If you don't have any httpgd client open, say you had run `hgd_close()`, then first run `hgd()` in R and an url will be shown in the R console. Then run `hgd_browse()` to open the url in the internal browser viewer.
Alternatively, use the command palette to run "**R Plot: Open httpgd Url**". It will let you enter the url address, fill it in and hit Enter to open the plot viewer. You can also copy and paste the url in an external browser.
...
<i class="codicon codicon-debug env-red" aria-hidden="true" style="font-size:1.5em; vertical-align: middle;"></i>
**Issue:** When you plot a new figure, it will show both in the VS Code plot panel and the browser viewer. 
It's like one device with two clients: the VS Code plot panel and the browser viewer. 
The browser viewer is more interactive and has better display quality, so I prefer to use the browser viewer. 

<i class="codicon codicon-lightbulb-sparkle env-green" aria-hidden="true" style="font-size:1.5em; vertical-align: middle;"></i> **Fix:** Open a plot directly in my httpgd browser without opening it inside VS Code plot panel. Add the following to your `.Rprofile`:
...
`viewer = TRUE` to open the plot viewer in a new internal WebView tab in VS Code.
Set `viewer = FALSE` will open the plot viewer in an external browser.


--------------------------------------------------------------------------------

**Commonly used functions in `httpgd`:** [↩](https://nx10.dev/httpgd/reference/index.html)
...
- `hgd_browse()` Open the plot viewer in your browser, can be internal or external.
  ...
  Click <i class="codicon codicon-link-external" aria-hidden="true" style="font-size:1.5em; vertical-align: middle;"></i> to open the viewer in an external browser.
  ...
  When you hover over the image, a menu bar will show up. You can choose to download, copy, <span class="env-green">clear all plots</span>, etc.
...
- `hgd_url()` will return the url of the viewer. 
  ...
  If you accidentally close the viewer, you can call `hgd_url()` to get the url and open it again.
  You can also copy and paste the url to your external browser.
...
**Show numbers in percent**

<span class="env-green">`scales::percent(x, accuracy)`</span> takes a numeric vector `x` and returns a *character* vector with the values formatted as percentages.

- `accuracy`: a number to round to. For example, `accuracy = 0.01` will round to the nearest 0.01 (1%).

```r
library(scales)
summary_stats %>% 
  mutate(across(min:sd, ~ percent(., accuracy = 0.01)))
```

will format columns between `min` and `sd` as percentages with 2 decimal places.

<div class="rmd-note">
Note: `scales` is a dependency of `tidyverse`, so you don't need to install it separately if you have `tidyverse` installed. However, you need to load it with `library(scales)` to use the `percent()` function. When you have `library(tidyverse)`, it won't load `scales` automatically, so you need to load it explicitly.
</div>

Another common use case is to format `ggplot2` axis labels as percentages. You can use `scale_y_continuous(labels = scales::percent_format)` to format the y-axis labels as percentages.
