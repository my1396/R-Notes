...
- For production documents, e.g., share results with coauthors, you may want to set `echo = FALSE` to hide the source code.
...
- You can put this snippet in a separate file, e.g., `setup.R`, and then include it in the first chunk of your Rmd via `source("setup.R")`.
  ...
  - `include=FALSE` is used so that in your output document, you won't see `source("setup.R")` and its output. ✅
  - If you use `echo=FALSE`, you won't see `source("setup.R")` but you will see the output of the code in `setup.R`, such as loading packages information.
...
- <span style="color: #00CC66;">`results='markup'`</span>: Defaults to `` `markup` ``. Mark up text output with the appropriate environments depending on the output format. 

  `results='markup'` means to put the text output in fenced code blocks (```` ``` ````), treat output **verbatim**. → Not interpreted as Markdown.

  Useful when you just want to show plain output, like from `print()`, `cat()`, or basic text generation.
...
- <span style="color: #00CC66;">`results='asis'`</span>: Write text output as-is, i.e., <span style="color: #00CC66;">write the raw text results directly into the output document without any markups.</span> → Output is interpreted as Markdown.
  ...
  **In summary, all formats will be applied.**
  ...
  **Other use cases:**
  ...
  You can use `results='asis'` to <span style="color: #00CC66;">read `.txt` files and include them</span> in the output document as-is. 

  ````markdown
  ```{r, results='asis'}
  cat("<pre style='white-space: pre-wrap;'>", paste(readLines("data_description.txt"), collapse = "\n"), "</pre>")
  ```
  ````
...
- Source `.R` scripts
  ...
  `include=FALSE` prevents the source code and its output from being included in the output document. This makes the output cleaner.
...
- Use the `file` option of a chunk.
  ...
  This method allows you to <span class="env-green">see the source code</span>, and it has proper syntax highlighting.
...
- Plain code blocks can be written either
  ...
  - After <u>three or more</u> backticks (fenced code blocks), or 
    ...
    Can also use <span class="env-green">**tildes** (`~`)</span>
...
- If your class is inside braces, you need to add a dot before the language name, e.g., `{.haskell}`. 
- If add the language name without the braces, you don't need the dot, e.g., `haskell`.
...
````markdown
```{r style="background: pink !important;"}
plot(cars)
```
````
...
```html
<pre class="r" style="background: pink !important;">
plot(cars)
</pre>
```
...
1. Define a CSS class `no-wrap` in your CSS file:
...
2. Add this class to the code block:
...
ref: [R Markdown Cookbook: Add attributes to text output blocks](https://pkg.yihui.org/rmarkdown-cookbook/attr-output)
