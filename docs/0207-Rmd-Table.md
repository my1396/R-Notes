...
`knitr::kable(x, format="pipe")` is useful when you want to copy-and-paste R output from console to markdown.
...
For Plain tables in txt, `knitr::kable(x, format = "simple")` is useful.


``` r
knitr::kable(mtcars[1:5, 1:5], format = "simple")
                      mpg   cyl   disp    hp   drat
------------------  -----  ----  -----  ----  -----
Mazda RX4            21.0     6    160   110   3.90
Mazda RX4 Wag        21.0     6    160   110   3.90
...
[`knitr::kable(x, format, caption=NULL, digits, escape=TRUE)`](https://bookdown.org/yihui/rmarkdown-cookbook/kable.html) Create tables in LaTeX, HTML, Markdown and reStructuredText.
...
- **`format`** Possible values are `latex`, `html`, `pipe` (Pandoc's pipe tables), `simple` (Pandoc's simple tables), `rst`, and `jira`. 
  ...
  When output is PDF, you can use `booktabs=TRUE` to use the `booktabs` package for better table formatting.
  ...
  `knitr::kable()` with no `format` argument will produce a markdown table, which Pandoc then converts to LaTeX or HTML depending on the output format.

  Pandoc by default wrap tables in `longtable` environment; this could collapse with other LaTeX macros, e.g., `adjustbox`. If you have `adjustbox` without specifying `format = "latex"`, you may get an error like:
...
- `caption` The table caption. 
  ...
  In order to number the table, mut specify the `caption` argument.
...
- <span class="env-green">`bootstrap_options`</span> A character vector for bootstrap table options; for <span class="env-green">**HTML** output only</span>.
  ...
  Possible options include `basic`, `striped`, `bordered`, `hover`, `condensed`, `responsive` and `none`.
...
- Print a raw table data
  
  You have to specify `summary = FALSE` to print the raw data instead of summary statistics. By default, `summary = TRUE`.

  `header = FALSE` suppresses the table header.
  ...
  `stargazer` will produce the following table in LaTeX output.
  ...
  & mpg & cyl & disp & hp & drat \\ 
  \hline \\[-1.8ex] 
  Mazda RX4 & $21$ & $6$ & $160$ & $110$ & $3.900$ \\ 
  Mazda RX4 Wag & $21$ & $6$ & $160$ & $110$ & $3.900$ \\ 
  Datsun 710 & $22.800$ & $4$ & $108$ & $93$ & $3.850$ \\ 
  Hornet 4 Drive & $21.400$ & $6$ & $258$ & $110$ & $3.080$ \\ 
  Hornet Sportabout & $18.700$ & $8$ & $360$ & $175$ & $3.150$ \\ 
  \hline \\[-1.8ex] 
  \end{tabular} 
  \end{table}
  ```
  ...
  <tr><td colspan="6" style="border-bottom: 1px solid black"></td></tr><tr><td style="text-align:left">Mazda RX4</td><td>21</td><td>6</td><td>160</td><td>110</td><td>3.900</td></tr>
  <tr><td style="text-align:left">Mazda RX4 Wag</td><td>21</td><td>6</td><td>160</td><td>110</td><td>3.900</td></tr>
  <tr><td style="text-align:left">Datsun 710</td><td>22.800</td><td>4</td><td>108</td><td>93</td><td>3.850</td></tr>
  <tr><td style="text-align:left">Hornet 4 Drive</td><td>21.400</td><td>6</td><td>258</td><td>110</td><td>3.080</td></tr>
  <tr><td style="text-align:left">Hornet Sportabout</td><td>18.700</td><td>8</td><td>360</td><td>175</td><td>3.150</td></tr>
...
- Passing a data frame to stargazer package creates a <span class="env-green">**summary statistic table**</span>. 
  ...
  <tr><td style="text-align:left">cyl</td><td>5</td><td>6.000</td><td>1.414</td><td>4</td><td>8</td></tr>
  <tr><td style="text-align:left">disp</td><td>5</td><td>209.200</td><td>100.256</td><td>108</td><td>360</td></tr>
  <tr><td style="text-align:left">hp</td><td>5</td><td>119.600</td><td>31.832</td><td>93</td><td>175</td></tr>
  <tr><td style="text-align:left">drat</td><td>5</td><td>3.576</td><td>0.422</td><td>3.080</td><td>3.900</td></tr>
  <tr><td colspan="6" style="border-bottom: 1px solid black"></td></tr></table>
