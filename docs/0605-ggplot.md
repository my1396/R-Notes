...
`theme_set()` accepts custom themes, e.g., <span class="env-green">`theme_set(my_theme)`</span> to set `my_theme` as the default theme for all subsequent plots.
...
```r
p + scale_x_continuous(limits = c(0, 100))  # clip data outside limits
p + coord_cartesian(xlim = c(0, 100))       # zoom in w/o clipping data
p + coord_fixed(ratio = 1, xlim = c(0,100)) # fixed aspect ratio
```
...
<span class="env-green">Add regression equation to scatter plot</span>

[`ggpubr::stat_regline_equation(label.y=NULL)`](https://search.r-project.org/CRAN/refmans/ggpubr/html/stat_regline_equation.html)
