...
- <span class="env-green">Share YAML configuration across multiple `.qmd` files.</span> 
  ...
  <div class="rmd-note">
  <p><code>_quarto.yml</code> will create a Quarto project and the YAML options will be applied to all in the project. </p>
  
  <p>If you want specific YAML options for a certian sub-directory, you can create a <a href="#directory-metadata"><code>_metadata.yml</code></a> file in that sub-directory.</p>
  </div>
...
- **Redirect output** to a specific folder. 
  ...
  `output-dir: docs` will redirect all output files to `docs` folder, which is useful for GitHub Pages.

- **Freeze rendered output**, so that only changed files will be re-rendered.
  
  `execute.freeze: auto` will create a `_freeze` folder to store cached results. When you run `quarto render`, Quarto will only execute code chunks in changed files, and will use cached results for unchanged files. This significantly speeds up rendering time for large projects.
...
1. Create a new `~/.quarto-cache/` directory on local disk,
2. Create a symbolic link `.quarto` in your project directory to point to the new cache directory. 

This way, when you render the project, the intermediate files will be stored in the new cache directory, and not synced to OneDrive. See [HERE](https://my1396.github.io/Econ-Study/2025/08/13/UgreenNAS-QuickStart.html#我的备份策略) for more details.
...
- Live preview. 
  ...
  This is <span class="env-orange">NOT</span> supported in Rmd.
...
- `_metadata.yml` will be merged to root `_quarto.yml` file (if there is one), and will override any conflicting options in the root `_quarto.yml` file. → You can have distinct metadata for different sub-directories in your project.
  
  `_metadta.yml` can exist <span class="env-green">without</span> a root `_quarto.yml` file. 
  
  Benefits of using `_metadata.yml`:
  
  - it allows you to specify metadata for a sub-directory without affecting the rest of the project. 
  
  - it will not create `.quarto/` cache folder, which can potentially cause issues with Cloud service syncing due to frequent changes and a large number of files tracked.
  
  - in short, `_metadata.yml` is lightweight and more specific. 
    
    By contrast, `_quarto.yml` is more general and applies to the whole project.
  ...
  and `_metadata.yml` can contain:

  ```yaml
  resources:
    - "../latex/filters/emoji/*.png"

  # Pandoc Extensions
  from: markdown+tex_math_single_backslash
  filters:
    - ../latex/filters/color-text.lua
    - ../latex/filters/nested-ordered-list.lua
    - ../../_shared-resources/filters/unlisted-sections.lua
    - ../latex/filters/emoji.lua
  format:
    pdf:
...
<a id="quarto-code-chunk-options"></a>
...
- Use code chunks to specify caption (`tbl-cap`) and float position (`tbl-pos`)
  ...
  ‼️ Do <span class="env-orange">NOT</span> use ~~`knitr::kable(caption)`~~ to specify caption. This will generate an error in Quarto.
...
# lua filters (1st level YAML)
filters:
  - ../filters/color-text.lua
  - ../filters/col-eqn.lua
  - ../filters/emoji.lua
  - ../filters/unlisted-sections.lua
  - ../filters/nested-ordered-list.lua
...
Use `hyperrefoptions` to specify options for the `hyperref` package. For example, to put the section number in front of each PDF bookmark, add the following to your YAML:
...
When you have <span class="env-orange">both HTML and PDF outputs</span>, you have to deal with particular details that are supported by html but not by pdf.
...
- For <span class="env-orange">`stargazer` tables</span>, use `type="text"` for best compatibility.
  
  Otherwise, use <span class="env-green">`type = ifelse(knitr::is_html_output(), "html", "latex")`</span> to specify different output types for html and pdf outputs.
...
<div class="rmd-caution">
Quarto has its own default MathJax (`mathjax@4/tex-chtml.js`), but when Quarto updates, it changes the default MathJaX version silently, it can break your existing documents. 
To avoid this, you can **pin a specific version** of MathJax in your `_quarto.yml` file.
</div>

In `_quarto.yml`, under `format: html:`:
...
My website was working fine with `colorV2` in MathJax 3. But after Quarto updated to using MathJax 4, the `colorV2` option was replaced with `colorv2`, and my website broke. 
So I decide to pin the MathJax version to 3.
...
<span class="env-green">`lang: en`</span> specifies the language of the document. This can affect hyphenation, spell checking, and other language-specific features in the output document. `en` is for American English; `en-GB` for British English.

See [HERE](https://quarto.org/docs/reference/formats/docx.html) for a complete list of metadata options for docx output.
...
- Render a Quarto **document** to HTML using the command line:
  ...
  - `--to` or `-t`: specify the output format(s). 
    
    You can specify multiple formats by separating them with commas, e.g., `--to html,pdf`. 
...
- You can also render a Quarto **project** using:
  ...
  If you are in the project directory, `quarto render` without specifying a file will render the entire project. This can be time-consuming (even with `freeze: auto`), use this when you are done with your edits and want to render the whole project for publishing.
...
- <span class="env-green">Quarto Preview</span>: display output in the **external web browser**.
  ...
  <div class="rmd-caution">
  If you two watcher on the same project, you will get an error like `ERROR: NotFound: No such file or direcotry (os error 2) readfile /Users/.../file-name.log`.
  
  Use `pkill -f 'quarto\.js preview'` to kill all the watchers and restart the preview.
  </div>



<div class="rmd-caution">
Q: How good is `quarto preview`?  
A: Use enormous amount of RAM; slow; keep refreshing but not showing the latest changes. 

I'd suggest a full re-render and open the html/pdf output ([⌘B](#open-html-in-internal-browser)) to see the changes. 
This is reliable and computationally efficient.
</div>

ref: [Quarto website preview server hacks](https://danmackinlay.name/notebook/quarto_website_preview_server.html)
...
- <span class="env-green">`--no-browser`</span> Use <span class="env-green">internal viewer</span>, do not open external browser.
...
<span class="env-green">`quarto preview [file] --port 7200 --no-browser`</span> Previewing website using a specific port. This is useful when your want to preview multiple documents simultaneously.
...
<a id="open-html-in-internal-browser"></a>
...
You can render your page and then ⌘R to refresh in your browser to see the changes. 👍
...
- If `input` is not specified, it will render the current Quarto project. If `input` is specified, it will render the specified Quarto document.
  ...
  By contrast, `rmarkdown::render()` only supports `.Rmd` files.
...
1. **Add labels**
   ...
   - **Code cell:** add option `label: prefix-LABEL` → 👍 most robust
   
   - **Markdown:** add attribute `#prefix-LABEL` → can fail, `#` can be tricky
...
See [HERE](#quarto-code-chunk-options) for more details about code chunk options.
...
**Prevent page breaks inside markdown tables**

Pandoc wraps markdown tables in a `longtable` environment for PDF output, which allows tables to span multiple pages. 
If you want to prevent page breaks inside a markdown table, use <span class="env-green">`\needspace`</span> to ensure enough space is available on the current page before the table starts.

~~~~markdown
```{=latex}
\needspace{10\baselineskip} % Adjust the number of lines as needed
```
Start your markdown table here:
...
`\needspace{10\baselineskip}` ensures that at least 10 lines of space are available on the current page before the table starts. 
The number of `\baselineskip` needs to be the number of lines in the table plus <span class="env-green">some buffer for the caption and spacing</span>.
If there isn't enough space, LaTeX will automatically move the table to the next page, preventing it from being split across pages. 

--------------------------------------------------------------------------------

<a id="quarto-stargazer-widetable"></a>
