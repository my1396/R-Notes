...
`predict.glm(object, newdata, type="link")` predict method for generalized linear models, e.g., logistic regression models fitted by `glm`.

- `type= "link" | "response"` specify the scale on which predictions are returned. The default is `"link"`, i.e., the scale of the linear predictors. 
