...
- `binwidth` 	<span class="env-green">**The width of the bins**</span>. Can be specified as a numeric value or as a function that calculates width from unscaled x. Here, "unscaled x" refers to the original x values in the data, before application of any scale transformation. When specifying a function along with a grouping structure, the function will be called once per group. 
...
- `center`, `boundary`  numeric values specify bin positions. One value for either center or boundary is adequate, other values will be automatically filled using `binwidth`.
  ...
  -  `boundary`   specifies the boundary between two bins. Boundary values are more informative. <span class="env-green">[suggest to specify one boundary value; just easier to say boundaries]</span>
...
- `after_stat(x)` uses variables calculated after the transformation by the layer stat (stage 2); 
  ...
  - `after_stat(count)` show frequncy count; <span class="env-green">`after_stat(ncount)`</span> count, scaled to a maximum of 1; 
