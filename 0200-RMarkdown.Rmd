...
- Do not run the code immediately, but run it later in another chunk. 
  ...
  **Code chunk 2** with option `ref.label='for-display'` will reuse the code from chunk 1, but it will be executed in this chunk.
  ...
  ```{r for-display, echo=TRUE, eval=FALSE}`r ''`
  library(stargazer)
  stargazer(
    mtcars[1:5, 1:5], 
  ...
  ```{r for-output, ref.label='for-display', echo=FALSE, eval=TRUE, results='asis'}`r ''`
  ```
  ````
...
````markdown
```{r setup, child = "_setup.Rmd", include=FALSE}`r ''`
```
````
...
````markdown
```{r, label="chunk-opt", include=FALSE}`r ''`
# set default chunk options
...
```{r eval=TRUE, echo=FALSE, out.lines = 4}
print(cars)
```
...
`` `r knitr::inline_expr("knitr::inline_expr('chunk_reveal('walrus', title='## Walrus operator')')")` ``
...
````markdown
```{r style="background: pink !important;"}`r ''`
plot(cars)
```
````
