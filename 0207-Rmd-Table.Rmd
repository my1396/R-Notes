...
```{r mtcars-simple, echo=TRUE, comment='', render=kat, class.output='md', collapse = TRUE}
...
- Print a raw table data
  ...
  ```{r stargazer-ex1, echo=TRUE, eval=FALSE}
  library(stargazer)
  stargazer(
    mtcars[1:5, 1:5], 
  ...
  ```{r stargazer-ex1-out, ref.label='stargazer-ex1', echo=FALSE, results='asis'}
  ```

- Passing a data frame to stargazer package creates a <span class="env-green">**summary statistic table**</span>. 

  ```{r stargazer-ex2, echo=TRUE, eval=FALSE}
  stargazer(
    mtcars[1:5, 1:5], 
    type = ifelse(knitr::is_html_output(), "html", "latex"),
    header = FALSE
  ...
  ```{r stargazer-ex2-out, ref.label='stargazer-ex2', echo=FALSE, results='asis'}
  ```
...
````markdown
```{r stargazer-html-pdf, echo=FALSE, results="asis"}`r ''`
library(stargazer)
...
````markdown
```{r sidewaystable-for-pdf, results="asis"}`r ''`
